...
data = [data[i] for i in torch.randperm(len(data), generator=g).tolist()]
...
C = torch.randn((len_chars, dimensions), generator=g)
W1 = torch.randn((dimensions * block_size, hidden_units), generator=g) * (5/3) / math.sqrt(dimensions * block_size)
h1 = torch.randn(hidden_units, generator=g) * 0.01
W2 = torch.randn((hidden_units, len_chars), generator=g) * 0.01
...
for i in range(max_steps):
  #foward pass
  ix = torch.randint(0, X_train.shape[0], (128,), generator=g)
  emb = C[X_train[ix]]
